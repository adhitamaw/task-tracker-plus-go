...
func (s *taskService) GetList(userID int) ([]model.Task, error) {
	tasks, err := s.taskRepository.GetList(userID)
	if err != nil {
		return nil, err
	}
	return tasks, nil
...
func (s *taskService) GetTaskCategoryByUser(categoryID, userID int) ([]model.TaskCategory, error) {
	taskCategories, err := s.taskRepository.GetTaskCategoryByUser(categoryID, userID)
	if err != nil {
		return nil, err
	}
	return taskCategories, nil
